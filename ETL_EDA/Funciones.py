...
def accidentes_mensuales(df):
    ...
    for i, year in enumerate(años):
        ...
        ax.set_facecolor('#226EE6')  # Cambia el color de fondo del gráfico
        ax.lines[0].set_color('#000000')  # Cambia el color de la línea del gráfico
...
def meses_con_mas_accidentes(df):
    ...
    plt.grid(axis='y', color='#EBCC52', linestyle='--', linewidth=0.5)
...
def cantidad_accidentes_por_categoria_tiempo(df):
    ...
    ax = sns.barplot(x='Categoria tiempo', y='Cantidad accidentes', data=data, color='#EB7D23')
    ...
    for index, row in data.iterrows():
        ax.annotate(f'{row["Cantidad accidentes"]}', (index, row["Cantidad accidentes"]), ha='center', va='bottom', color='#EA1405')

    # Se agrega el color de fondo al gráfico
    ax.set_facecolor('#E0EBA0')
...
def accidentes_tipo_de_calle(df):
    ...
    sns.countplot(data=df, x='Tipo de calle', ax=axes[0], color='#EB7D23')
    axes[0].set_title('Cantidad de víctimas por tipo de calle') ; axes[0].set_ylabel('Cantidad de víctimas')

    sns.countplot(data=df, x='Cruce', ax=axes[1], color='#EB7D23')
    ...
    axes[0].set_facecolor('#E0EBA0')  
    axes[1].set_facecolor('#E0EBA0')  
...
def cantidad_accidentes_por_tipo_acusado(df):
    ...
    # Se crea el gráfico de barras
    plt.figure(figsize=(10, 6))
    ax = sns.barplot(x='Tipo de Vehículo', y='Cantidad de Accidentes', data=data, color='#EB7D23')
    ...
    # Se agrega las cantidades en las barras
    for index, row in data.iterrows():
        ax.annotate(f'{row["Cantidad de Accidentes"]}', (index, row["Cantidad de Accidentes"]), ha='center', va='bottom', color='#EA1405')
...
def cantidad_victimas_sexo_rol_victima(df):
    ...
    # Gráfico 1: Sexo
    sns.countplot(data=df, x='Sexo', ax=axes[0], color= '#EB7D23')
    ...
    df_rol.plot(kind='bar', stacked=True, ax=axes[1], color=['#1730FA', '#8C2CFA'])
    ...
    df_victima.plot(kind='bar', stacked=True, ax=axes[2], color=['#1730FA', '#8C2CFA'])
...
def victimas_por_rango_etario(df):
    ...
    return grupos['Edad'].size().plot.bar(figsize=(10, 6), color=['#8407EB', '#EBBE1E', '#1EEBBA', '#07EB1A', '#1D26EB'],
